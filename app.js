...
app.view('job_channel_select', async ({ ack, body, view }) => {
  try {
    ...
    console.log('About to acknowledge and update view');
    ...
    // Make sure we acknowledge even on error to prevent Slack timeout
    try {
      await ack({
        response_action: "update",
        view: {
      ...
      console.error('Error acknowledging on failure:', ackError);
    }
  }
});
...
app.view('materials_select_modal', async ({ ack, body, view }) => {
  try {
    console.log('Materials selected by user:', body.user.id);
    ...
    if (selectedMaterials.length === 0) {
      ...
      console.log('No materials selected, showing error');
      await ack({
    ...
    if (selectedMaterials.length > 20) {
      console.log('Too many materials selected, showing error');
      await ack({
        response_action: "errors",
        errors: {
    ...
    console.log('About to acknowledge and update view for quantity entry');
    ...
    // Make sure we acknowledge even on error
    try {
      await ack({
        response_action: "update",
        view: {
...
// -------- STEP 4: Review Modal --------
app.view('quantity_entry_modal', async ({ ack, view, body }) => {
  try {
    console.log('Quantities entered by user:', body.user.id);
    ...
    if (hasError) {
      console.log('Validation errors in quantities:', errorBlocks);
      await ack({
        response_action: "errors",
        errors: errorBlocks
      });
    ...
    console.log('About to acknowledge and update view for review');
...
app.view('review_modal', async ({ ack, body, view, client }) => {
  try {
    console.log('Review submitted by user:', body.user.id);
    await ack();
    ...
    try {
      await client.chat.postMessage({
        ...
        text: `❌ Sorry, there was an error submitting your materials list: ${error.message}`
      });
    } catch (notifyError) {
